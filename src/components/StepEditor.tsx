...
import { AlertCircle, Crosshair, KeyRound, MousePointer2, Plus, Trash2 } from "lucide-react";
import {
  WorkflowStepSchema,
  locatorKinds,
...
export function StepEditor({ step, onUpdate }: StepEditorProps) {
  ...
  const updatePayload = (key: string, value: unknown) => onUpdate({ ...step, payload: { ...step.payload, [key]: value } } as WorkflowStep);
  ...
  return (
    <div className="step-editor-content">
      <div className="editor-section action-editor">
        <div className="editor-section-title"><MousePointer2 size={15} /><span>Action details</span></div>
      ...
      <div className="editor-section locator-editor">
        <div className="editor-section-title"><Crosshair size={15} /><span>Locator candidates</span><small>{candidates.length}</small></div>
        {step.target ? (
          <div className="locator-list">
            {candidates.map((candidate, index) => (
              <div className="locator-row" key={`${candidate.kind}-${index}`}>
                <span className="locator-rank">{index + 1}</span>
                <label><span className="sr-only">Locator kind</span><select value={candidate.kind} onChange={(event) => updateCandidate(index, { kind: event.target.value as LocatorCandidate["kind"] })}>{locatorKinds.map((kind) => <option value={kind} key={kind}>{kind}</option>)}</select></label>
                <label className="locator-value"><span className="sr-only">Locator value</span><input value={candidate.value} onChange={(event) => updateCandidate(index, { value: event.target.value })} /></label>
                {candidate.kind === "role" ? <label className="locator-name"><span className="sr-only">Accessible name</span><input value={candidate.name ?? ""} placeholder="Name" onChange={(event) => updateCandidate(index, { name: event.target.value })} /></label> : null}
