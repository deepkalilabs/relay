...
function reviewAdrBoundary(root, branch, checkpoint) {
  ...
  command("npm", args, { cwd: root, stdio: "inherit" });
}
...
async function runPlan(planInput) {
  ...
  const plan = repoPath(root, planInput);
  if (!plan.local.startsWith("docs/plans/") || extname(plan.local) !== ".md") {
  ...
  const slug = slugForPlan(plan.local);
  const branch = expectedBranch(plan.local);
  ...
  const codeIntel = await startCodeIntel(root, stateDirectory);
